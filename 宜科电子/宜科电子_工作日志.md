...
现在的项目情况：
在多次和甲方沟通后，确认了下周一的13点和19点开始培训。
这周主要是保持每天的陪产工作，以及每日和罗世成的沟通工作，用于确保项目在咱们软件层面方面的万无一失。
...
```txt
【GRJ2105_ChangChunPaintShop】
甲方马克加了需求，我方修改了停台通报和Audit录入页面的功能，包括数据库，前后端
```

##### 2023年11月9日周4

```txt
整理项目代码，统一命名为"project-序号-项目名称"
```
...
```txt
和陈名兴学习成都沃尔沃的项目架构以及业务代码，该项目使用的是昊方硬件。
```
...
```txt
【GQC2335||大庆沃尔沃总装物流PTL项目】
1.学习昊方的demo，以及硬件软件的调试安装视频。梳理成了安装调试文档(用word编写)
2.和陈名兴学习成都沃尔沃的程序架构，业务代码，SQLSERVER架构和数据，为大庆项目做准备

预计下周：开始做出差沃尔沃的相关准备（体检，装备等）
```
...
```txt
【GJY2204_江西理工大学机电工程学院智能制造项目】
项目处于“验收阶段”，配合现场的李成伟完成相关的工作

【GQC2335||大庆沃尔沃总装物流PTL项目】
做去现场前的准备工作（装备，体检，流程，熟悉项目流程）
```
...
中国国航CA8225
天津滨海T2-大庆萨尔图
15:30-17:20【应该是17:30，提前了10分钟】,14:49登机
```

##### 2023年11月28日周2

```txt
1.完成进入沃尔沃的考试，培训等甲方安排的事
2.熟悉场地，一共有三块区域要投入使用【其中X-Gate的变化很大，主要是外型和重量上。不知道是否会给施工带来难度】
3.和许工【群里的柠檬微甜🍋】，滕工，IT沟通现场的需求变动，数据库相关事宜等工作

明天要问陈工，页面的代码具体在哪，该做啥工作。
```

##### 2023年11月29日周3

```txt
开发配置程序的新增批量导入功能【Excel数据导入】
```
...
```txt
1.在晚上，先对四个总线的每一台设备（6位屏，彩屏），设置地址
2.一起排查总线1没有通信的原因，最后查到是第二排位置边缘的扁缆短路
3.进行压力测试，目前每一条总线能支持35+的设备同时运行
```

##### 2023年12月3日周7

```txt
在凌晨方便施工的时候，按现场的FLOW以及零件号，更新PTL物料信息
```

#### 周报

```txt
1.完成前期工作进入现场，并熟悉硬件位置，软件程序。
2.和甲方沟通，确认需求，以及合作中所需的服务器，电脑，IP等
3.在周末熟悉LC区域的BUS线布局，记录更新PTL项目的物料信息。
```
...
```txt
1.完成批量导入，新增Flow规则的功能开发
2.录入了107个现场的设备，完成3号箱XGate相关信息录入
```
...
```txt
在晚班进行了录入1，2号箱的一共128个设备的工作。
```

##### 2023年12月8日周5

```txt
1.在沃尔沃1206服务器部署接收程序。发现了英文系统log日志乱码的问题，正在尝试解决
2.更新因“甲方标签变动”而发生物料Flow变化的标签
```
...
```txt
预期工作：
1.解决接收程序日志在英文系统乱码的问题
2.配置剩余14个XGate对应的标签设备（6位屏/彩屏），一共17个XGate
...
```txt
预期：
完成配置剩余14个XGate对应的标签设备（6位屏/彩屏），一共17个XGate

15:00-03:00：
1.配置4,5,6号XGate对应的物料，共270个
2.做每个箱子的压力测试
3.合力检查解决有问题的标签设备电路，完成压力测试
```
...
【GQC2335||大庆沃尔沃总装物流PTL项目】
下发，接收程序（后台逻辑部分）：
             1.根据甲方给予的”物料文件“和”Flow文件“，配置PTL_INfo表所需的各种信息。
               【本周共记录了503个标签设备，剩余350个左右。】
			 2.在沃尔沃提供的服务器做好了服务注册以及程序的运行。
             3.解决了一些英文系统导致的乱码和文件缺失的问题
配置程序（用户配置页面）：
		1.和甲方产线负责人沟通需求，确认改造的部分
        2.一共有5个需求，其中3个已经开发和测试完毕。剩余2个是在更新时新加字段，较为简单

其他：1.配合陈工，和沃尔沃甲方沟通服务器事宜以及确认协议
     2.为了配置XGate的“正确率”，只向PTL_Info录入已经通电的设备。所以要先等待施工方配电完成。
```
...
```txt
1.根据现场FLow位置。按顺序配置PTL_INFO以及PTL_GATEWAY_INFO
2.解决可能存在的虚接问题
3.配合实施同事解决扁缆短路，铝板变形，端子铁片缺失等现场的电路问题【主要问题点，花的时间最多】
```
...
接收程序：运行情况良好
下发程序（后台逻辑部分）：
             1.解决英文系统中，中文排序规则报错的问题
             2.按现场的XGate顺序，BUS线顺序更新PTL_INFO以及PTL_GateWayINFO
             3.固定添加PTLControler，和XGate数量保持一致
             4.为满足甲方对每个Flow订单顺序的定制，给Flow添加存储过程
配置程序（用户配置页面）：
		1.跳单功能，新增mixnumber字段
		2.添加Flow功能，新增两个字段，“是否有后排设备”，“Flow的订单等待时间”
```
...
##### 2023年12月19日周2 至 2023年12月26日周2

```txt
先看了下肝胆，肝胆老是疼。是脂肪肝，没有结石，只是碰到了肝的膜。
但是感染了支原体，病了一周多。现在还没彻底好，现在时间是2023年12月27日09:41:27
```
...
1.【完成】批量导入
2.【完成】Flow对应的排序规则
3.当前彩屏是否后排也有Flow，等待时间WaitTimeSeconds
4.【完成】跳单加上mixnumber
```
...
```txt
天津出差大庆，15:30至17:30飞机途中。
```

##### 2023年12月30日周6

```txt
1.根据现场情况更新【大庆沃尔沃PTL项目信息汇总】Excel
2.更新文档【大庆沃尔沃总装物流排序拣选系统常见故障处理指导书】和【大庆沃尔沃总装物流排序拣选系统操作指导书】
3.开发配置程序，更新SQLSERVER相关的表以及存储过程
4.为甲方各Flow负责人培训"标签设备"和"配置程序"的使用方法和架构
```
...
```txt
1.解决现场LC区域4箱121的设备软件方面的问题，
解决现场LC区域6箱60的设备软件方面的问题，
解决现场LC区域1箱Flow：FPR，零件号：31434527的设备软件方面的问题，
2.更新现场新增或者修改位置的标签设备。如：新加件13箱1线的31479193，地址新增1线4
3.修改程序订单等待时长，把等待时间设置为10秒
4.新增报警需求：当发生"零件缺失"，"mix号不连续"时报警
5.新增Excel导出需求，给PTL管理页面新增这个功能
6.删除【添加Flow】页面的”特殊彩屏“，”高低位“，”有无后排“
7.新增字段无后排isBackRow（默认是否），特殊彩屏isColorScreen（默认是否），两个字段的形式做成下拉，分别是（是/否），录入数据库是1/0,显示是/否。这两个字段放到PTL管理
8.书写完成2023年的年度工作总结
9.根据甲方提供的缺失的PTL编码，核对现有的PTL管理的信息数据
```

##### 2024年1月3日周3

```txt
1.处理区域门线1的FLow为DHR，以及Flow为TMK，CSD的设备故障
2.修改Flow为CSD和TMK区域的订单等待时长，改为3秒
3.检查05箱58和61两个地址的设备同时灭的问题，检查后发现是误点，因为两个完成时间有个0.5s的延迟
4.在现场培训甲方的维修人员，设备的一些基本操作和维修
5.PNC，RSS，TMK增加了排灯时长，
6.解决IT的接收程序掉线导致的订单和现场不同步的问题【全场溜】

【硬件】
根据甲方下图提供的一些角度，制作培训教程以及后续工作，包括：
1.粘贴IP地址
2.电源地址不清的粘贴照片
3.维修培训：(1).接线盒 (2).X-GATE箱 (3).标签 (4).配置 (5).常见故障
```

##### 2024年1月4日周4

```txt
1.RSS的彩屏位置从地址78换到地址71
2.统计PTL_INFO数据，转发甲方ME许工
3.GUF，GUR，GSE，GSA修改拍灯时长
```

##### 2024年1月5日周5

```txt
1.解决IP区域【DAQPTS-AX13	10.240.97.157】不亮灯的问题，到发生地后，重启了XGate解决了问题。
2.玻璃房 AX09-111 设备烧了，换了个新的
```

##### 2024年1月6日周6

```txt
1.发现玻璃房，门线-1有设备虚接的问题。经检查，发现虚接现象是有滑轨的箱子砸到板子引起的
2.解决“ax05-128  拍完彩屏以后，又亮了，彩屏是129”的问题。跳单解决该问题
3.因为Ax09也就是玻璃房经常会出现设备烧坏的问题。和昊方沟通后，在1月6日晚上更换了XGATE
4.ax09玻璃房上午9:29烧了一个设备
```

##### 2024年1月7日周日

```txt
1.完成报警功能。零件缺失，mix号不连续的时候报警
2.提醒陈工：定期删除表的数据，如：告警，其他
3.检查4台电脑设备的网络以及使用情况。对有问题的网络端口做记录
```

##### 周报

```txt
1.按甲方要求，培训PTL检料系统的软件的使用和配置方法
2.解决“大量”的现场问题，主要问题原因是：硬件设备，硬件线路，设备虚接，中英文转码乱码
3.根据甲方要求，更新相关的现场标签数据，IP数据等数据表
4.按甲方提出的新需求，更新程序以及数据库相关的数据
5.在出现硬件问题的时候，联系现场的施工部门同事排查以及解决问题。
```
...
```txt
1.解决RDD，RDE，RDF三个FLOW的订单和标签设备不一致的问题。经检查是中文在英文系统乱码导致的，已解决。
2.为RDC*1，LC*2，玻璃房*1共4台电脑进行沃尔沃内部的安全更新。这几台机器无法设置远程桌面，只能独立使用。
```

##### 2024年1月9日周2

```txt
先给维修组进行软件培训，包括下发程序和接收程序。之后又用1206做了程序展示和配置程序操作展示
```

##### 2024年1月10日周3

```txt
1.和甲方沟通现场的问题。同步宜科的企业邮箱给甲方
2.发现DPR的31485904在订单没有出现，但是却亮了
3.处理配置程序的报错。解决BUG。把新程序同步给生产环境
```
...
```txt
1.给各个机柜进行安全更新
2.沟通2024年2月份的成都物流项目改造。确认参与该项目
3.更换玻璃房的XGATE。并且重启程序
```

##### 2024年1月12日周5

```txt
解决RDC区域的机柜安装安全补丁失败的问题。我把机箱搬到了LC，并且更改了IP和网关，然后更新安全补丁
```

##### 2024年1月13日周6

```txt
1.再次出现了周3第二条的问题。dpr79单出现问题：零件号31485904，ax10-67。在这个单没有，但是亮了
2.按照现场负责人的需求，新增标签设备【都是彩屏】。更新了生产数据库的PTL_INFO.
```
...
```txt
1.根据甲方的需求，同步现场的标签设备
2.根据甲方的需求，为维修组进行了3次培训，且每次开会均有签字
3.处理下发程序，配置程序在英文环境下出现的问题。
4.处理RDC区域机柜无法安装安全补丁的问题
5.处理DPR区域新增设备显示和订单不同步的问题。
6.已解决WAR频繁烧毁设备的问题。
```
...
```txt
1.解决DPR订单和现场不同步的问题【跳单解决】
2.解决GRI现场灯不亮的问题【修改存储过程】，原因是修改71个存储过程给漏了一个
```
...
```txt
停产歇半天，下午去的
下午主要和西门子售后刘海友，以及陈名兴一起搞的4号机更新问题
```
...
```txt
1.AX05号XGate-122,126，AX09号112，共3台设备损坏。更换新设备。
2.定期观察昊方的PtlDeviceLog文件夹的硬件Log日志。
```
...
计划任务：
配置程序
【急切开发】1.在【添加Flow】加个TapLightTimeSecs，记录拍灯等待时间
2.添加【历史记录查询】，只能查询七天之前的数据。导出留下。起始时间+查询按钮
【急切开发】3.添加【生产状态】页面，两个字段FLOW的id，Status状态。记录产线的停止和开启。1是停止，0是开启
4.在【添加Flow】页面，加上新规则"分层播种"
```

##### 2024年1月21日周7

```txt
【注意：以后insert，delete，Update之前一定要备份数据库】
1.来现场把scancode_record和scancode_record_Detail的数据导入到History历史表中。中间出了点问题。插入的时候条件给少了。
2.开发配置程序
```
...
```txt
【特别事项】
1. 现场坏了太多设备，生产能换的设备就1个了
2. 频繁的设备损坏一直找不到问题。实施工程师吕工各种检测的方式都试过了，也没有得到解决。他已经和昊方联系了。
昊方说是周一来

完成的工作：
1.解决了几个影响现场生产的程序错误。
2.解决了4号机柜无法安全更新的问题【手动安装补丁】，已经通过沃尔沃IT安全扫描
3.定期观察昊方的PtlDeviceLog文件夹的硬件Log日志。AX05号XGate-122,126，AX09号112，共3台设备损坏。更换新设备。

未来的工作：
1.和陈工沟通新需求对现有表结构的改动，存储过程的改动。
2.做好界面的交互逻辑，配合好陈工的下发程序
...
```txt
1.玻璃房 地址104设备日志发现坏了;
设备坏了。104是在没有发数据的情况下烧掉的，因为插上以后，并没有亮
2.甲方给【生产状态】页，加了点需求。现在要把班组加上。下拉选择班组，然后选择Flow
```

##### 2024年1月23日周2

```txt
1.昊方的项目经理来了，来了解现场的情况。给出了解决方案。
    【以下是邮件信息：包工:您好
    问题现象:1、PTL项目内一个小区域电子拣货标签损坏频繁;2、两台控制器出现过死机，间隔时间2周左右。
    PTL料架现场实地检查后，情况如下
    理论上，表面接线线路没问题，电子拣货标签安装没问题，控制器接线也没问题。
    损坏产品检测情况如下:
    均为电源芯片损坏，公司会把芯片返回厂家检测分析，检测时间大概2周左右出结果。
    解决办法:1、用14个最新批次的产品更换小区域的电子标签，2天内发出;
    2、控制器死机问题再观察一段时间，如果还会出现死机情况，立刻换新】
2.配合产线负责许工，对她的同事进行现场培训和软件使用的培训
3.确认甲方给出的修改方案，和对PTL管理和生产状态两个页面的需求
```
...
```txt
1、今天没有虚接的设备
2. 开发配置程序的生产状态，班次配置页面
3. 开发定时任务，用于定时删除现场数据。减少标签响应时间
```
...
```txt
1.【配置程序】修改导入功能，完善生产状态页面
2.【SQLSERVER】完成定时任务Job的开发。经过现场查看后，保证定时任务的稳定运行。解决设备拍灯数据累积导致的拍灯迟缓问题。
```
...
周报总结
硬件部分
  1.设备故障:玻璃房的地址104设备在未发送数据的情况下损坏，插电后无响应。后续未报告新的设备损毁，但周三发现了一个虚接的设备问题。
  2.现场问题解决:昊方项目经理针对PTL项目问题提出解决方案，包括更换电子拣货标签和观察控制器死机问题，后者将继续监控。

软件部分
  1.根据甲方需求，开发并完成了【生产状态】页面的班次配置功能。
  2.页面开发:开发了PTL管理页面，并对发现的功能BUG进行了测试和修复。
  3.开发SQL定时任务，用于定时删除现场数据。减少标签响应时间
  4.客户需求调整: -对于甲方的新需求，加入了班组的选择功能至【生产状态】页面。
               -配合产线负责人许工，对同事进行了现场培训和软件使用的培训。

Weekly Summary

Hardware Section:
- Equipment Failure: The device at location 104 in the Glass Room was damaged without sending data, with no response when powered. No new equipment damages were reported after this incident, but a loose connection was identified on Wednesday.
- On-Site Problem Resolution: The Haofang project manager proposed solutions for the PTL project issues, including replacing electronic picking labels and monitoring the controller's freezing problem, which will continue to be observed.

Software Section:
- Feature Development and Optimization: Developed and completed the shift configuration feature on the 【Production Status】 page according to client requirements.
- Page Development: Developed the PTL management page and tested and fixed identified functional bugs.
- Customer Requirement Adjustments: Added the ability to select teams on the 【Production Status】 page in response to new client demands.
- Training Support: Cooperated with the production line manager, Xu Gong, to provide on-site and software usage training to colleagues.
```
...
```txt
1.培训-现场物流班长配置程序中生产状态的使用
2.寄快递-将前几天的坏件【标签设备】寄给昊方给的地址，进行返修或者更换
```

##### 2024年2月1日周4

```txt
1.修复-现场挪动标签出现虚接，修复针脚重插后解决
2.收件-XGATE到现场了，去1号门门口拿的
```

##### 2024年2月2日周5

```txt
1.维护-【原因是误触了Fun键导致的人员识别错误】解决现场DPR区域的问题。
2.实施-更换97.157网络异常的XGATE
3.准备工作-拷贝生产环境的PTL_DB_DQ数据库
```
...
2024年2月5日周1~2024年2月15日周4（初6）请假+放假
...
```txt
1.到现场熟悉工作区域，保密守则，可行区域等
2.登记现场的标签设备，更新Excel
3.对“玻璃房”的标签设备压力检测。调整产线和标签设备【解决虚接问题，保证运行正常】
```

##### 2024年2月18日周7

```txt
1.在文件上，登记现场物料的设备标签信息。【完成了SQ11-SQ12，SQ12-SQ13；时刻防止交叉作业的产生】
2.根据文件，更新物料文件Excel
```
...
```txt
1.在文件上，登记现场物料的设备标签信息。【完成了SQ13-SQ14，SQ14-SQ15，SQ15-SQ16；时刻防止交叉作业的产生】
2.根据文件，更新物料文件Excel
```
...
```txt
1.在文件上，登记现场物料的设备标签信息。【完成了SQ16-SQ17，SQ17-；时刻防止交叉作业的产生】
2.根据文件，更新物料文件Excel
```
...
```txt
计划任务：
	1.更新ptl_info,baseInfo,gatewayINfo
	2.为配置程序新增规则【播种，顺序】

实际完成：
1.配置现场标签设备；
2.使用昊方软件进行压力测试；
3.调试标间设备，使现场350个设备都能顺利完成压力测试
```
...
```txt
开发成都沃尔沃配置程序。新增PTL导出，现场检料规则配置功能；
```
...
```txt
配合陈工调试现场的标签设备。对多次出现虚接的问题件，问题总线记录，和实施工程师沟通该问题。
```
...
```txt
第三周（2月16日至2月18日）
1.成都出差：         2月16日（周五）到达成都
2.现场熟悉与设备登记：2月17日（周六） 熟悉了工作区域及保密守则，完成了现场标签设备的初步登记，并更新了Excel记录。同时，对“玻璃房”内的标签设备进行了压力测试，并调整了产线以确保设备正常运行。
3.设备标签信息记录：2月18日（周日），继续在文件中登记现场物料的设备标签信息（涵盖了从SQ11至SQ13的序列号），并且更新了物料文件Excel，注重预防交叉作业。

第四周（2月19日至2月25日）
1.持续登记与更新：      2月19日（周一）至2月20日（周二），我完成了从SQ13至SQ17的设备标签信息登记，并同步更新了Excel文件。
2.现场配置与压力测试：2月21日（周三）至2月22日（周四），我配置了现场标签设备并使用昊方软件进行了压力测试，确保350个设备均能通过。
3.程序开发与规则配置：2月23日（周五）和2月24日（周六），我着手开发成都沃尔沃配置程序，增加了PTL导出和现场检料规则配置功能。
4.设备调试与问题沟通：2月25日（周日），与陈工合作调试标签设备，记录并沟通了频繁出现的虚接问题。
```
...
```txt
1.修改现场物料对应的FLow配置，更新最新的Flow名
2.开发配置程序，新增库位字段，保证甲方能顺利核对物料的具体总线位置
```
...
1.完成PTL管理页面，库位功能的发布。
2.修改操作指导书
3.解决PTL界面查询后，点击编辑“报错：无实例对象”的问题。【因为ptl_sel_query存储过程没有加上Store_Location导致的】
```
...
```txt
1.给甲方沃尔沃RDC区域的物流负责做系统培训，包括：软件的使用，硬件的使用，以及故障出现后的应对方法。
2.成铸【RDC区域的物流负责】提出需要系统崩溃的应急方案。
```

##### 2024年3月5日周2

```txt
接到【成都沃尔沃物流拣选系统】的甲方负责人-ME经理-潘挺【英文名gus】的要求，让我们实现应急预案。
```

##### 2024年3月6日周3

```txt
在PTLAPP开发【实时备份功能】，【一键还原】功能。用于备份每日最新的配置文件，在系统崩溃时用于还原配置数据
```
...
```txt
1.为甲方ME经理-潘挺【英文名gus】，成铸【RDC区域的物流负责】培训PTLAPP【实时备份功能】，【一键还原】功能。
2.更新现场的程序以及存储过程，使新功能正常使用，并备份原版软件。
3.完成应急方案文档的编写。交于陈名兴，罗世城。
```
...
```txt
1.去现场测试大庆的软件，调试把设备标签写成动态读取ptlinfo，而不再是固定的flow和设备了
2.同时保证更换软件后，成都下发程序的稳定运行
```

##### 周报

```txt
1.（系统培训与应急准备）完成了对【沃尔沃RDC区域物流负责】成铸的系统培训，包括软件、硬件使用及故障处理，以及根据他和【ME经理】潘挺的要求，开始了应急预案的实施，提高了遇到系统问题时的响应效率。
2.（PTLAPP功能开发）开发了PTLAPP的实时备份和一键还原功能，这将显著增强数据安全性，并确保系统的快速恢复。
3.（功能培训与程序更新）对【ME经理】潘挺和【RDC区域物流负责】成铸进行了PTLAPP新功能的培训，并更新了现场程序和存储过程，确保新功能的顺畅运行和系统的稳定性。
4.（应急方案文档）完成了应急方案文档的编写，并已提交给陈名兴及罗世城，以便进一步审查和备案。
```
...
```
1.安排给me经理潘挺(gus)的培训
2.培训高士琪（成铸的助理）对新功能自动备份，一键还原以及处理设备故障的方法。
```
...
```txt
1.编写交接清单给成铸
2.再次培训PTL物流负责人成铸。包括配置程序和下发程序的使用，以及文档的交接
```
...
```txt
1.备份成都沃尔沃723k程序。和陈工同步数据库，配置程序，下发程序
2.回公司报销
```
...
```txt
GRJ2311|成都沃尔沃PTL项目，新增日志删除功能，做到冗余数据的定时清理，节省空间
```

##### 2024年3月22日周5

```txt
GQC2335||大庆沃尔沃总装物流PTL项目，修改PTL管理页面的存储过程，不再限制多个零件号对应一个地址的情况。
```
