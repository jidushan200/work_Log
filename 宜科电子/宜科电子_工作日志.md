...
--判断实际数据(additional is null)是否包含文件名
   ...
   declare @STR nvarchar(200),@codestr nvarchar(1000)

   set @codestr = '';
   declare cur_tr5 cursor 
   for (select id
			from(select *,row_number () OVER (PARTITION BY [code] ORDER BY [id]) AS 'num' from @tab_result2  where [additional] is null)t
				where t.num = 1)
   open cur_tr5
   ...
   begin
		...
		--调试
		--select * from @output where charindex(@STR,line) > 0
		--select @STR

		--查找是否存在该文件
		if (select COUNT(*) from @output where charindex(@STR,line) > 0) = 0
		begin
			set @code = (select code from tm_result where id = @id)
			if LEN(@codestr) > 0
				begin
					set @codestr = @codestr + ',' + @code
				end
			if LEN(@codestr) = 0
				begin
					set @codestr = @code
		...
		--set @Path='D:\ftpdata\' --获取路径下的文件
		--if OBJECT_ID('tempdb..#FileText') is not null drop table #FileText; --获取共享目录中的文件列表
		--create table #FileText
		--	(
		--		id int identity ,  --编号
		...
		--declare cur_tr6 cursor
		--for(select [filename] from #FileText)
		--open cur_tr6
		--fetch next from cur_tr6 into @filename
		--while @@FETCH_STATUS = 0
		--begin
		--	select @filename '文件名'
		--	select CHARINDEX(@ip,@filename) '是否包含ip（参1：查找字符，参2：字符串）'; --是否包含ip（参1：查找字符，参2：字符串）
		--	fetch next from cur_tr6 into @filename
		--end
		--close cur_tr6;
		--deallocate cur_tr6;
